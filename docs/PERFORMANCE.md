...
| Main-thread parsing | Spreadsheet parsing runs in a worker (Rust/Wasm engine); PDF uses the PDF.js worker |
...
## Spreadsheet Wasm engine

Measured on the release build (`wasm-pack build --release --target web`, wasm-opt `-Oz`) of the `spreadsheet-view` crate:

| Artifact | Measured size |
| --- | --- |
| `packages/viewer/dist/spreadsheet/spreadsheet_view_bg.wasm` | 730 kB |
| `packages/viewer/dist/spreadsheet/spreadsheet_view.js` (glue) | 17 kB |

The artifact is loaded lazily by the spreadsheet worker only when a spreadsheet opens. The main `anydoc_wasm_bg` conversion binary is unaffected.
