...
5. Job, error, artifact, and sink-result values use AnyDoc's portable persistence grammar: `null`, booleans, finite numbers, strings, dense arrays, and plain objects with enumerable string keys. Binary is represented only as `{ "$anydoc": "binary/base64", "data": "..." }`. `undefined`, non-finite numbers, bigint, functions, symbol values, cycles, accessors, sparse arrays, and class/platform objects such as `Date`, `Map`, `Set`, and `Blob` are rejected. Symbol-keyed host metadata is outside the grammar and is not inspected or persisted, which keeps traversal allocation-bounded. Do not persist credentials, parser instances, signals, or native render models.
